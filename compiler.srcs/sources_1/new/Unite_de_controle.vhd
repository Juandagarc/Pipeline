...
begin
    ...
    if rising_edge(pipeline_clk) then
        ...
        --split instr mem output 
         DI_opcode <= instruction(31 downto 24);
         DI_op1 <= instruction(23 downto 16);
         DI_op2 <= instruction(15 downto 8);
         DI_op3 <= instruction(7 downto 0);
      
        --move everything one stage further in the pipeline (EX op2: determined in mux_di)
         EX_opcode <= DI_opcode; 
         EX_op1 <= DI_op1;
         EX_op3 <= REG_Q_B;
         
         --move everything one stage further in the pipeline (ME_op2: mux_ex)
         ME_opcode <= EX_opcode;
         ME_op1 <= EX_op1;
        ...
        --increase programm counter idk how to do that
